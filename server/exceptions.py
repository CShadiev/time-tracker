...
class PermissionDeniedError(HTTPException):
    def __init__(self, detail: str | None = None):
        if detail is not None:
            ...
            _detail = ""
        super().__init__(status_code=403, detail=_detail)


class UserExistsError(HTTPException):
    def __init__(self, username: str):
        super().__init__(status_code=400, detail=f"Username {username} taken")


class TokenExpiredError(HTTPException):
    def __init__(self):
        super().__init__(status_code=401, detail="Access token expired")


class MissingTokenError(HTTPException):
    def __init__(self):
        super().__init__(status_code=401, detail="Missing access token")
...
class ProjectNotFoundError(DBObjectNotFoundError):
    def __init__(self, key: str):
        super().__init__(status_code=404, detail=f"Project {key} not found")
...
class TaskNotFoundError(DBObjectNotFoundError):
    def __init__(self, key: str):
        super().__init__(status_code=404, detail=f"Task {key} not found")
